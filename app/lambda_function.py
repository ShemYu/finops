...
def get_creator_of_instance(instance_id, region, event_state, lookback_days=7):
    ...
    resp = ct.lookup_events(
        LookupAttributes=[{"AttributeKey": "EventName", "AttributeValue": event_state_attribute_value_maps[event_state]}],
    ...
    for event_rec in resp.get("Events", []):
        ...
        items = evt.get("responseElements", {}) \
                   .get("instancesSet", {}) \
                   .get("items", [])
        ...
        if instance_id in ids:
            ...
            return {
                "time": event_rec["EventTime"].astimezone(timezone(timedelta(hours=8))).strftime("%Y%m%d %H:%M:%S"),
                "user_arn": user,
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if DEBUG:
            ...
            creator_info = get_creator_of_instance(event["detail"]["instance-id"], event["region"], event["detail"]["state"])
            block = send_ec2_event_to_slack(instance_info, creator_info, event["detail"]["state"])
            send_message(block)
